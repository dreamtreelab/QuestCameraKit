...
namespace MTAssets.EasyMeshCombiner.Editor
{
    public class MeshCombinerTool : EditorWindow
    {
        ...
        */
        ...
        private string MERGE_RESULTS_SUFFIX = "(EMC Merge)";
        ...
        public static void OpenWindow()
        {
            ...
            window.minSize = new Vector2(620, 676);
            window.maxSize = new Vector2(620, 676);
        ...
        void OnGUI()
        {
            ...
            Texture arrowIcon = (Texture)AssetDatabase.LoadAssetAtPath("Assets/Plugins/MT Assets/Easy Mesh Combiner/Editor/Images/Arrow.png", typeof(Texture));
            Texture arrowDoneIcon = (Texture)AssetDatabase.LoadAssetAtPath("Assets/Plugins/MT Assets/Easy Mesh Combiner/Editor/Images/ArrowDone.png", typeof(Texture));
            ...
            if (validsGameObjectsSelected.Count > 0 && isAllMergeParamsValids == true)
            {
                ...
                if (mergeIsDone == true)
                {
                    ...
                    GUILayout.BeginVertical();
                    GUILayout.Space(7);
                    if (GUILayout.Button("New Merge", GUILayout.Width(80), GUILayout.Height(49)))
                    {
                        //Save the preferences
                        SaveThePreferences(this);
        ...
        void UI_MergePreferences_AllInOne()
        {
            ...
            if (currentPipelineOfProject != RenderPipelineCurrentlyBeingUsed.BuiltIn && meshCombinerPreferences.allInOneParams.useDefaultMainTextureProperty == true)
            {
                ...
                {
                    meshCombinerPreferences.allInOneParams.mainTexturePropertyToFind = "_MainTex";
                    meshCombinerPreferences.allInOneParams.mainTexturePropertyToInsert = "_BaseColorMap";
                }
            }
            if (currentPipelineOfProject == RenderPipelineCurrentlyBeingUsed.BuiltIn && meshCombinerPreferences.allInOneParams.useDefaultMainTextureProperty == true)
            {
        ...
        private void ValidateGameObjectsSelection()
        {
            ...
            for (int i = 0; i < gameObjectsWithMeshFound.Count; i++)
            {
                ...
                if (thisMeshRenderer != null)
                {
                    ...
                    {
                        logsOfBeforeMerge.Add(new LogOfMerge(MessageType.Warning, "The mesh in GameObject \"" + thisGameObject.name + "\" contains many vertices and a various materials. Due to a Unity limitation, you may experience a longer time to combine the meshes with Lightmap Support. If this happens, try reducing the amount of meshes present in this merge."));
        ...
        private static void LoadThePreferences(MeshCombinerTool instance)
        {
            ...
            meshCombinerPreferences = (MeshCombinerPreferences)AssetDatabase.LoadAssetAtPath("Assets/Plugins/MT Assets/_AssetsData/Preferences/EasyMeshCombiner.asset", typeof(MeshCombinerPreferences));
            ...
            if (meshCombinerPreferences != null)
            {
                ...
                {
                    AssetDatabase.DeleteAsset("Assets/Plugins/MT Assets/_AssetsData/Preferences/EasyMeshCombiner.asset");
            ...
            if (meshCombinerPreferences == null)
            {
                ...
                AssetDatabase.CreateAsset(meshCombinerPreferences, "Assets/Plugins/MT Assets/_AssetsData/Preferences/EasyMeshCombiner.asset");
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();
            }
        }
            ...
            foreach (TexturesSubMeshes item in texturesAndSubMeshes)
            {
                ...
                if (currentTextureIndexOfCurrentAtlas == meshCombinerPreferences.allInOneParams.maxTexturesPerAtlas || totalOfTexturesAndSubMeshesLeft <= 0)
                {
                    ...
                    try
                    {
                        //Try to do the merge...
                        bool hasLightmapData = ((meshCombinerPreferences.lightmapSupport == true && meshCombinerPreferences.lightmapMode == MeshCombinerPreferences.LightmapMode.RecycleData) ? true : false);
                        submesh.CombineMeshes(allCombineInstancesOfThisAtlas.ToArray(), true, true, hasLightmapData);
                    }
                    catch (Exception ex)
                    {
                        //Check if is a error of Uint16
                    ...
                    if (meshCombinerPreferences.allInOneParams.emissionMapSupport == true)
                    {
                        ApplyAtlasInPropertyOfMaterial(material, meshCombinerPreferences.allInOneParams.emissionMapPropertyToInsert, atlasData.emissionMapAtlas);
                        allTextures2dOfFinalMergedMesh.Add(atlasData.emissionMapAtlas);
                    }
        ...
        {
            if (show == true)
                EditorUtility.DisplayProgressBar("A moment" + ((meshCombinerPreferences.lightmapSupport == true) ? " (Using Lightmaps Support)" : ""), message, progress);
        ...
        private void GenerateLightmapDataForMesh(ref Mesh targetMesh)
        {
            //Show progress bar
            ShowProgressBar("", false, 0.0f);
            ShowProgressBar("This may take SEVERAL minutes! Generating Lightmap Data for Combined Mesh...", true, 1.0f);

            //Wait time
            Thread.Sleep(100);

            //Create the UnwrapParam
            UnwrapParam unwrapParams = new UnwrapParam();

            //If is desired to use custom values
            if (meshCombinerPreferences.lightmapUseDefaultGenerationParams == false)
            {
                unwrapParams.angleError = meshCombinerPreferences.lightmapParamAngleError;
                unwrapParams.areaError = meshCombinerPreferences.lightmapParamAreaError;
                unwrapParams.hardAngle = meshCombinerPreferences.lightmapParamHardAngle;
                unwrapParams.packMargin = meshCombinerPreferences.lightmapParamPackMargin;
            }

            //If is desired to use default values
            if (meshCombinerPreferences.lightmapUseDefaultGenerationParams == true)
                UnwrapParam.SetDefaults(out unwrapParams);

            //Generate the secondary UV for the target mesh
            bool result = Unwrapping.GenerateSecondaryUVSet(targetMesh, unwrapParams);

            //Warn if have a error
            if (result == false)
                logsOfBeforeMerge.Add(new LogOfMerge(MessageType.Error, "Unable to generate Lightmap Data for the mesh resulting from the merge. Check the Console for more details."));
        }
        ...
        private Dictionary<Texture2D, bool> ExtractReferenceOfAllTexturesThatWillBeUsed(Material[] materialsThatWillBeUsed)
        {
            ...
            foreach (Material material in materialsThatWillBeUsed)
            {
                ...
                if (emissionMap != null && allTextures.ContainsKey(emissionMap) == false)
                    allTextures.Add(emissionMap, emissionMap.isReadable);
            }
        ...
        private Texture2D GetSimpleCopyOfTexture(Material materialToFindTexture, string propertyToFindTexture, int widthOfCorrespondentMainTexture, int heightOfCorrespondentMainTexture, TextureType textureType, float progress, int texturesProcessedAtHere, int totalTexturesForProcess)
        {
            ...
            //-------------------------------------------- Create a reference to target texture
            ...
            //-------------------------------------------- Convert this simple texture copy, to a normal map texture if the type is normal map
            //If this texture should be a Normal Map, convert this texture to be a linear texture
            if (textureType == TextureType.NormalMap)
            {
                //Recreate the texture in Normal Map format
                Color[] thisTextureDefaultPixels = thisTexture.GetPixels();
                thisTexture = new Texture2D(thisTexture.width, thisTexture.height, TextureFormat.RGBA32, thisTexture.mipmapCount, true);   //<- Enable the linear color space, that is needed for Normal Map textures
                thisTexture.SetPixels(0, 0, thisTexture.width, thisTexture.height, thisTextureDefaultPixels);
            }
